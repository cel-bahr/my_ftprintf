...
/*   Updated: 2024/02/20 01:54:59 by cel-bahr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_printf(const char *format, ...)
{
    int i;
    int lenght;
	//a pointer to iterator on the arguments of the list 
    va_list arg;
    
	//protect standardoutput if closed or in general in case of failure of write
    if (write(1, "", 0) < 0)
        return (-1);
		//initilize arg to point on the last variable here is format;
    va_start(arg, format);
    i = 0;
    lenght = 0;
    while (format[i])
